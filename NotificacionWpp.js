...
async function enviarAlertaWhatsapp(mensaje) {
  for (const to of toNumbers) {
    try {
      ...
      logger.info(`[${new Date().toLocaleString()}] 📲 WhatsApp enviado a ${to}: ${result.sid}`);
    } catch (err) {
      logger.error(`[${new Date().toLocaleString()}] ❌ Error enviando a ${to}: ${err.message}`);
    }
  }
}

// 🕒 Versión v2 de Cloud Scheduler
exports.verificarMonitor = onSchedule(
  {
  ...
  async (event) => {
    try {
      ...
      logger.error(`[${new Date().toLocaleString()}] ❌ ERROR en el monitor: ${err.message}`);
